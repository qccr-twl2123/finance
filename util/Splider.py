...
m2_list = map(lambda x: str(x.string).strip(), soup.find_all("td", class_="xl55"))
m2_list = m2_list + map(lambda x: str(x.string).strip(), soup.find_all("td", class_="xl57"))
m2_list = m2_list + map(lambda x: str(x.string).strip(), soup.find_all("td", class_="xl59"))

m1_list = map(lambda x: str(x.string).strip(), tr[2].find_all("td", class_="xl63"))
m1_list = m1_list + map(lambda x: str(x.string).strip(), tr[2].find_all("td", class_="xl64"))
